...
argument-hint: A review target (a design-system a11y spec, an implemented screen/flow, a UI diff, or a pre-release scope) plus the relevant packet sections (§10, with §12/§3/§13) — or, for edit mode, an explicit task to author a named accessibility artifact such as an accessibility conformance report (VPAT/ACR).
