...
argument-hint: One AI/LLM feature from the approved design (the model-using capability, its prompt/eval/guardrail requirements, the business rules it must honor, the data it may send to the model, and the cost constraint), plus the bundle task and the provider adapter interface owned by the Integration Engineer.
