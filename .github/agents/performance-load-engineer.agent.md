...
argument-hint: A performance review target (the services or journeys to load, the budget/SLO scope, or a pre-release performance scope) plus the relevant packet sections (§11 scale & reliability, §3 journeys, §13 acceptance) and any approved design or observability signals it traces to.
