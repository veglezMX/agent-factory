...
# Roo Code — native single .roomodes (customModes: array)
scripts/install.sh --target roo --scope project --path /path/to/your/project   # -> <dir>/.roomodes (recommended)
scripts/install.sh --target roo   # global -> Roo's custom_modes.yaml in the editor's globalStorage (auto-detected)
