...
2. **Global vs project:**
   - **Project** (`--scope project --path <dir>`) writes `<dir>/.roomodes` at the workspace root — Roo reads it natively.
   - **Global** (`--scope global`, the default) writes Roo's real global modes file, `custom_modes.yaml`, inside the editor's VS Code globalStorage for the Roo extension (`…/globalStorage/rooveterinaryinc.roo-cline/settings/`) — **not** `~/.roomodes`. The script auto-detects that dir for VS Code, VS Code Insiders, VSCodium, Cursor, and Windsurf, and writes to every editor that has Roo installed. If none is found it fails with the candidate paths; open Roo once so the dir exists, set `ROO_SETTINGS_DIR=<that settings dir>` to point at it explicitly, or fall back to a project install. Reload the editor window after installing.
